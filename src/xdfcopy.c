...
void format_track(int fd, int drive, int track, int head, struct xdf_struct *fmt,
...
#define NEXT(x,max) x+=2; if(x>=max)x=1;
	...
	if ( track == 0 ){
		...
		skew = (track * fmt->skew) % fmt->period;
		max=0;
...
void read_sectors(enum fs fs, int sector, char **data, int sectors,
		  int drive, int fd, int track, struct xdf_struct *fmt, 
...
void readwrite_track(int fd, int drive, enum dir direction, 
		     int track, char *data, struct xdf_struct *fmt,
		     int FatSize, int RootDirSize)
{
    ...
    if(track) {
	    ...
	    read_sectors(MAIN_FS, 0, &data, 1 + FatSize,
			 drive, fd, track, fmt, direction, raw_cmd, &j);
...
int main(int argc, char **argv)
{
    ...
    fmt = xdf_table + type;
    for ( track = 0 ; track < max_cylinder ; track++){
	    if(sourcename) {
		    if ( sdrive == - 1 ){
			    ...
			    readwrite_track(sfd, sdrive, READ_, track, buffer, fmt,
					    FatSize, RootDirSize);
			    clear();
		    }
	    } else {
	    ...
	    if (tdrive == -1){
		    ...
		    if(!noformat) {
			    ...
			    for(head = 0; head < 2; head++)
				    format_track(tfd, tdrive, track,head, fmt,
						 FatSize, RootDirSize);
			    clear();
		    }
		    if(progress) {
		    ...
		    readwrite_track(tfd, tdrive, WRITE_, track, buffer, fmt,
				    FatSize, RootDirSize);
		    clear();
	    }
    }
